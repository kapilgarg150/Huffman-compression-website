...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	else
	{
		...
		$infix = '('.$infix.')';
		$prefix_value = prefix_exp_only(strrev($infix));
	}
}
	...
	for($i = 0;$i< strlen($input);$i++) 
	{
		...
		else if(is_operator($input[$i])) 
		{
			...
			while(!$temp->isEmpty() && $temp->top() != ')' && precedence($temp->top(),$input[$i]))
			{
				$output.= $temp->top();
				$temp->pop();
				only_once($temp,$input,$i,strrev($output),$k);
				$k++;
			}
			$temp->push($input[$i]);
			if($k==0)
				stack($temp,$input,$i,strrev($output));
			else
				stack($temp," ",0,strrev($output));	
		}
...
<br><br>
<?php if(!empty($_POST["infix"]))$postfix_value = prefix_exp(strrev($infix)); ?>
</body>
</html>